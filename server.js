...
io.on('connection', (socket) => {
  ...
  socket.on('voice:join', ({ auth, roomCode } = {}, cb) => { try { const acc = requireAuth(auth); const room = socketRoomOf(socket, roomCode); const p = ensurePlayer(room, acc); if (!voiceRooms.has(room.code)) voiceRooms.set(room.code, new Map()); voiceRooms.get(room.code).set(p.id, socket.id); socket.join(`voice:${room.code}`); emitVoiceList(room.code); cb?.({ ok: true, peers: voiceParticipants(room.code).filter((x) => x.playerId !== p.id) }); } catch (e) { cb?.({ ok: false, error: e.message }); } });
  socket.on('voice:leave', ({ auth, roomCode } = {}, cb) => { const acc = authUser(auth); const room = rooms.get(String(roomCode || '').toUpperCase()); const p = room && acc ? getPlayerByAccount(room, acc.id) : null; if (room && p && voiceRooms.has(room.code)) { voiceRooms.get(room.code).delete(p.id); socket.to(`voice:${room.code}`).emit('voice:peer-left', { playerId: p.id }); emitVoiceList(room.code); } cb?.({ ok: true }); });
  socket.on('voice:signal', ({ roomCode, toPlayerId, data } = {}) => { const room = rooms.get(String(roomCode || '').toUpperCase()); const pmap = room && voiceRooms.get(room.code); const targetSocketId = pmap?.get(toPlayerId); if (targetSocketId) io.to(targetSocketId).emit('voice:signal', { fromPlayerId: [...pmap.entries()].find(([, sid]) => sid === socket.id)?.[0], data }); });

  socket.on('disconnect', () => { for (const room of rooms.values()) { const p = room.players.find((x) => x.socketId === socket.id && !x.left); if (p) { p.socketId = null; p.connected = false; p.disconnectedAt = Date.now(); addLog(room, `${p.name} terputus.`, 'system'); const vr = voiceRooms.get(room.code); if (vr) { vr.delete(p.id); emitVoiceList(room.code); } emitRoom(room.code); } } saveRooms(); });
});
...
function emitVoiceList(code) { io.to(`voice:${code}`).emit('voice:list', { participants: voiceParticipants(code) }); }
